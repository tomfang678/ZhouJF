$(function () {
    ...
    function getRegion() {
        $.ajax({
            type: "GET", url: "/Home/GetRegions", data: { cityid: l }, dataType: "json",
            success: function (t) {
                for (var f, i = 0; i < t.length; i++)
                    f = new drawCricle(t[i].Name, t[i].Lat, t[i].Lng), n.addOverlay(f), $(f.domElement).click(function () {
                        ...
                        n.centerAndZoom(new BMap.Point(t, i), 10);
                        e(r, u)
                    })
            }
        })
    ...
    function e(i, r) {
        $(".inputName").blur(); var e = $(".sel_map").width(), s = $(".loadMap").width(),
            f = (e - s) / 2, u = d(), h = u.bssw.lng, l = u.bsne.lng, y = u.bssw.lat, p = u.bsne.lat; $(".loadMap").css({ left: f + "px" }); $(".loadpop").css({ display: "block" }); $(".notHoust").css({ display: "none" }); b(); c.mapserach(h, l, y, p, i, r, function (u) {
                var e, s; if (u.HasResult) {
                    for ($(".notHoust").css({ display: "none" }), $(".house_list").attr("data-count", u.RoomCount), u.RoomCount <= 15 && $(".page button").attr("disabled", "disabled"), e = 0; e < u.Communities.length; e++) {
                        s = new w(u.Communities[e]); n.addOverlay(s); $(s._wrapDiv).hover(function () { var n = this; t = setTimeout(function () { $(n).find(".houseInfo").css({ "z-index": 1e3 }); $(n).find(".toggleDiv").fadeOut(0); $(n).find(".houseInfo").fadeIn(0); $(n).find(".houseInfo").attr("data-id") == o && $(n).find(".houseInfo").css({ display: "block", cursor: "pointer" }) }, 200) }, function () {
                            var n = this; clearTimeout(t);
                            $(this).find(".houseInfo").css({ "z-index": null }); $(n).find(".houseInfo").fadeOut(0); $(n).find(".toggleDiv").fadeIn(0); $(n).find(".houseInfo").attr("data-id") == o && $(n).find(".houseInfo").css({ display: "block" })
                        }); $(s.houseInfo).on("click", function () {
                            var t, n, u; isClick = !1; $(".house_list").scrollTop(0);
                            t = o = $(this).attr("data-id"); n = $(this).attr("data-remainihouse");
                            $(".house_list").attr("data-count", n); console.log("count" + n); n <= 15 ? ($(".page button").attr("disabled", "disabled"), $(".page button").css({ background: "#a0a0a0", color: "#f0f0f0" })) : ($(".page button").removeAttr("disabled"), $(".page button").removeAttr("style")); u = 1; i = $(".price").attr("data-pos-cfg") || 1; r = $(".cfg").attr("data-pos-cfg") || 1; $(".houseInfo").css({ display: "none" }); $(".house_list").find("div").removeClass("house-pos-display"); $(this).find("div").attr("class", "house-pos-display"); c.SearchRooms(t, u, i, r, function (n) { console.log(n); n.RoomCount > 0 ? a(n.Rooms) : v() })
                        })
                    } $(".loadpop").css({ display: "none" })
                } else n.clearOverlays(), $(".loadMap").css({ left: f + "px" }), setTimeout(function () { $(".loadpop").css({ display: "none" }) }, 800), setTimeout(function () { $(".notHoust").css({ display: "block" }) }, 800); a(u.Rooms)
            })
    }
    /*搜索功能*/
    function it() {
    ...
    function k() { $(".sel_map").css({ dipslay: "block", "margin-left": "350px" }); $(".serach_map_house").fadeIn(200); $(".serach_house_name").fadeIn(200) }
    ...
    function a(n) {
        var t = [], i, r; if (n !== undefined) {
            for (i = 0; i < n.length; i++) t.push('<div class="serach_house_item">'),
                t.push("<a href=/RoadBasicInfo/Detail?ID=" + n[i].ID + ' target="_blank" class="mulch"><\/a>'), t.push("<a href=" + n[i].pictureThuemb + ' target="_blank">'),
                t.push('<div class="house_preview" style="background:url(' + n[i].pictureThuemb + ') no-repeat;background-size:100% 100%;">'),
				t.push("<\/div>"), t.push('<div class="house_info">'),
				t.push("<label>"), t.push(n[i].RoadName),
				t.push("<\/label>"), t.push("<label>"),
				t.push(n[i].ExtendURL), t.push("<\/label>"),
				t.push("<label>"), t.push("<em>" + n[i].RoadLevel + "<\/em> "),
				t.push("<em>" + n[i].RoadMaterial + "<\/em> "),
				t.push("<\/label>"), t.push("<label>"),
				t.push("<span>" + n[i].RoadCode + "</span><span>桥梁" + n[i].BridgeNumber + "座<\/span>"),
				t.push("<\/label>"),
				t.push("<\/div>"),
				t.push("<\/a>"),
				t.push("<\/div>");
    ...
    function v() {
        ...
        $(".inputName").val() === "" ? n.push('<p class="housing">没有检索到信息<\/p>') : n.push('<p class="housing">请选择区域<' + $(".inputName").val() + "><\/p>");
    ...
    var c = {
        ...
        mapserach: function (n, t, i, r, u, f, e, o) { this._post("/Home/SearchCommunities", { fromlng: n, tolng: t, fromlat: i, tolat: r, price: u, cfg: f }, e, o) },
        GetSearchTips: function (n, t) { this._post("/Home/GetSearchTips", { cityid: n }, t) },
        SearchRooms: function (n, t, i, r, u) { this._post("/Home/SearchRooms", { communityIds: n, page: t, price: i, cfg: r }, u) }
    ...
    n.centerAndZoom(new BMap.Point(s, h), 11); n.enableDragging();
    ...
    drawCricle.prototype.initialize = function (n) {
        var t, i; return this._map = n, t = this._div = document.createElement("div"),
            t.style.position = "absolute", t.style.background = "url('../../Assets/map/area1.png') no-repeat",
    ...
    w.prototype.initialize = function (n) {
        ...
        return this._map = n, o = this._wrapDiv = document.createElement("div"), $(o).attr("class", "posHouseDiv"), o.style.position = "absolute",
            ...
            f.style.cursor = "pointer", h = document.createElement("span"), h.style.fontWeight = "normal", h.appendChild(document.createTextNode("共计")),
            ...
            s = document.createElement("span"), s.style.fontWeight = "bold", s.style.color = "#ff5300", s.style.fontSize = "12px",
            ...
            o.appendChild(i), $(h).find("em").after("条道路"), l.appendChild(f), f.appendChild(s), s.appendChild(c),
            //y = $(f).find("span").after("起"), 
			i.appendChild(e),
			$(i).find("p").after('<span class="sanjiao1"><\/span>'),
			n.getPanes().labelPane.appendChild(o), o
    };
