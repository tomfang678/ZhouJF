        ...
        bootbox.confirm("确定删除？删除后将无法恢复。", function (result) {
            if (result === true) {
                $.ajax({
                    ...
                    dataType: 'text',
                    cache: false,
                    url: '/RoadBasicInfo/DeleteDtlById',
...
function pageReady() {
    ...
    $("#sure").click(function () {
        if ($('#formEdit').validate().form()) {
            ...
            //唯一性验证
            $.ajax({
                url: "/RoadBasicInfo/CheckCode?CODE=" + $("#txtSubContractorCode").val(),
                type: "post",
                dataType: "json",
                async: false,
                success: function (data) {
...
function CommitForm() {
    $.ajax({
        ...
        success: function (data) {
            if (data.IsSuccess) {
                window.location.href = "/RoadBasicInfo/Index";
            } else {
                if (data.ErrorCode == -1) {
