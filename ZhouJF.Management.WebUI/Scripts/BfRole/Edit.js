...
$(document).ready(function () {
    $.ajax({
        ...
        dataType: 'text',
        cache: false,
        url: '/BfRole/GetAllOperationByUserId?roleId=' + $("#hidRoleId").val(),
        success: function (data) {
            var html = "";
            ...
            for (var i = 0; i < resultData.length; i++) {
                ...
                //通用权限
                for (var j = 0; j < resultData[i].CurrentBfModuleOperationInfos.length; j++) {
                    var operationItem = resultData[i].CurrentBfModuleOperationInfos[j];
                    if (operationItem.IsSelect) {
                        html += operationItem.MODULE_NAME + "<input type=\"checkbox\" name=\"chkOperation\" checked=\"checked\" value=\"" + operationItem.ID + "\" />&nbsp;&nbsp;&nbsp;&nbsp;";
                    }else
                ...
                html += "<td style=\"vertical-align:middle\">";
                //特殊权限
                for (var j = 0; j < resultData[i].CurrentBfModuleOtherOperationInfos.length; j++) {
                    var otherOperationItem = resultData[i].CurrentBfModuleOtherOperationInfos[j];
                    if (otherOperationItem.IsSelect) {
                        html += otherOperationItem.NAME + "<input type=\"checkbox\" name=\"chkOtherOperation\" checked=\"checked\" value=\"" + otherOperationItem.ID + "\" />&nbsp;&nbsp;&nbsp;&nbsp;";
                    } else {
