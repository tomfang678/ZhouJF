...
{ "mDataProp": null, "mMaxWidth": "150px", "sWidth": "250px" }],
        "columnDefs": [
            {
                ...
                "render": function (data, type, row) {
                    ...
                    if ($("#hidIsEdit").val() == "True")
                        html.push("<a href=\"/SubContractorInfo/Edit/{0}\" class=\"btn default btn-xs green\"><i class=\"fa fa-edit\"></i> 编辑 </a>");
                    else {
    ...
    $("#btnImportTemplate").click(function () {
        window.open("/SubContractorInfo/ImportTemplate");
    });

    $("#btnImport").click(function () {
        $("#basic").data("actionRrl", "/SubContractorInfo/Import");
        $("#basic").data("errorRrl", "/SubContractorInfo/GetImportError");
    });

    $("#btnExport").click(function () {
        var str = FormTools.getQueryStringFromForm("#searchContatiner");
        window.open("/SubContractorInfo/Export?" + str);
    });
}
...
function del(el, id) {
    bootbox.confirm("确定删除,删除以后将无法恢复该数据和相关联的数据?", function (result) {
        if (result === true) {
            $.ajax({
                url: "/SubContractorInfo/Delete/" + id,
                type: "POST",
                dataType: "json",
